...
function MapView({ friends = [], MBTI_GROUPS }) {
  ...
  const locationStats = useMemo(() => {
    ...
    friends.forEach(friend => {
      const location = friend.province;
      if (!location) return;
  ...
  const getOption = () => {
    const data = Object.entries(locationStats).map(([name, stats]) => {
      // 将省份简称转换为地图需要的完整名称
      const mapName = PROVINCE_NAME_MAP[name] || name;
      return {
        name: mapName,
        value: stats.total,
  ...
  const selectedLocationFriends = useMemo(() => {
    ...
    return locationStats[selectedProvince]?.friends || [];
  }, [locationStats, selectedProvince]);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">
              {selectedProvince ? `${selectedProvince}的好友` : '全国省份分布'}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {selectedProvince
                ? `点击地图查看其他省份 · 点击右下角返回全国`
                : '点击省份查看该省好友详情'}
            </p>
          </div>
        </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">加载地图数据中...</p>
          </div>
        </div>
      )}
      ...
      {/* 选中省份的好友列表 */}
      {selectedProvince && selectedLocationFriends.length > 0 && (
        <div className="mt-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-5 border border-blue-100">
          <div className="flex items-center gap-2 mb-4">
            <Users className="text-blue-500" size={20} />
            <div>
              <h3 className="text-lg font-bold text-gray-800 dark:text-white">{selectedProvince}的好友</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">共 {selectedLocationFriends.length} 位</p>
            </div>
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <img
                      ...
                      alt={friend.mbti}
                      className="w-12 h-12 object-contain flex-shrink-0 bg-white dark:bg-gray-800/20 rounded-lg p-1"
                    />
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <Users className="text-blue-500" size={20} />
            <span className="text-sm font-medium text-gray-600 dark:text-gray-300">覆盖地区</span>
          </div>
          <p className="text-2xl font-bold text-gray-800 dark:text-white">{Object.keys(locationStats).length} 个</p>
        </div>

        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <MapPin className="text-purple-500" size={20} />
            <span className="text-sm font-medium text-gray-600 dark:text-gray-300">总好友数</span>
          </div>
          <p className="text-2xl font-bold text-gray-800 dark:text-white">{friends.length} 位</p>
        </div>
      </div>
